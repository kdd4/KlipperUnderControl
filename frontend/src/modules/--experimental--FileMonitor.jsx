...
export function FileMonitor({ navigate }) {
  ...
  const handleDrop = (targetItem, e) => {
    e.preventDefault();
    setIsDragging(false);
    
    if (!draggedItem || !targetItem || !targetItem.isDir || draggedItem.name === targetItem.name) {
      return;
    }

    // Перемещаем элемент
    const sourcePath = `${currentPath}/${draggedItem.name}`;
    const targetPath = `${currentPath}/${targetItem.name}/${draggedItem.name}`;
    
    moonrakerAPI.moveFile(sourcePath, targetPath)
      .then(() => {
  ...
  const handleDropToRoot = (e) => {
    ...
    // Перемещаем элемент в корень текущей директории
    const sourcePath = `${currentPath}/${draggedItem.name}`;
    const targetPath = `${currentPath}/${draggedItem.name}`;
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      await moonrakerAPI.createDirectory(folderPath);
      setShowCreateFolderModal(false);
      setNewFolderName('');
      await loadDirectory(currentPath);
    } catch (err) {
      setError(`Ошибка создания папки: ${err.message}`);
    }
  };
  ...
  return (
    ...
    {showCreateFolderModal && (
      <div className="action-modal-overlay" onClick={() => setShowCreateFolderModal(false)}>
...
function FileGrid({ items, onSelect, onNavigate, getIcon, isGcodeFile, selectedPath, 
  ...
  return (
    <div 
      className={`file-grid ${isDragging ? 'dragging-active' : ''}`}
      onDragOver={onDragOver}
      onDragEnd={onDragEnd}
      onDrop={handleDropToRoot} // Обработчик дропа в корень
    >
      ...
      {/* Индикатор для создания папки при перетаскивании */}
      {isDragging && (
        <div 
          className="file-tile drop-indicator"
          onDragOver={onDragOver}
          onDrop={handleDropToRoot}
        >
          <div className="tile-icon">📁</div>
          <div className="tile-name">Переместить сюда</div>
        </div>
      )}
    </div>
  );
