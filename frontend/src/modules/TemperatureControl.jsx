...
export function TemperatureControl() {
  ...
  const handleSetTemp = async (type) => {
    ...
    try {
      ...
      // В реальном приложении здесь будет запрос к PHP
      console.log(`Setting ${type} temp to ${targetTemp[type]}°C`);
      
      // Имитация запроса
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // После успешной установки можно обнулить поле
      setTargetTemp(prev => ({ ...prev, [type]: 0 }));
    } catch (error) {
      console.error('Error setting temperature:', error);
    } finally {
  ...
  return (
    <div className="temperature-control">
      <div className="temp-control-group">
        <h4>Экструдер</h4>
        <div className="temp-presets">
          {presetTemps.extruder.map(temp => (
            <button 
              key={temp}
              className="preset-button"
              onClick={() => setTargetTemp(prev => ({ ...prev, extruder: temp }))}
            >
              {temp}°
            </button>
          ))}
        ...
        <div className="temp-input-row">
          <input
            type="number"
            value={targetTemp.extruder}
            onChange={(e) => setTargetTemp(prev => ({
              ...prev,
              extruder: Number(e.currentTarget.value)
            }))}
            min="0"
            max="300"
            placeholder="0"
          ...
          <button 
            ...
            onClick={() => handleSetTemp('extruder')}
            disabled={isLoading.extruder}
          >
            {isLoading.extruder ? 'Установка...' : 'Применить'}
          </button>
        </div>
      </div>
      ...
      <div className="temp-control-group">
        <h4>Стол</h4>
        <div className="temp-presets">
          {presetTemps.bed.map(temp => (
            <button 
              key={temp}
              className="preset-button"
              onClick={() => setTargetTemp(prev => ({ ...prev, bed: temp }))}
            >
              {temp}°
            </button>
          ))}
        ...
        <div className="temp-input-row">
          ...
          <button 
            ...
            disabled={isLoading.bed}
          >
            {isLoading.bed ? 'Установка...' : 'Применить'}
          </button>
        </div>
      </div>
    </div>
